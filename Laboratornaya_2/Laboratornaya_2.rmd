...
df1 <- data.frame(Year = numeric(), Period.Desc. = character(), Reporter = character(), Netweight..kg.model = numeric())
df2 <- data.frame(Year = numeric(), Period.Desc. = character(), Reporter = character(), Netweight..kg.model = numeric())
for (year in c(2013, 2014, 2020)){
  for (m in month.name[1:6]){
    df1 <- rbind(df1, data[data$Year == year & str_detect(data$Period.Desc., m), ])
  }
  for (m in month.name[7:12]){
    df2 <- rbind(df2, data[data$Year == year & str_detect(data$Period.Desc., m), ])
  }
}
...
df.embargo <- df.embargo[df.embargo$Weight < 2e+05, ]

# Строим график и сохраняем его в директорию где расположен скрипт
png('image_ggplot.png', width = 1000, height= 1000)
ggplot(data = df.embargo, aes(x = Weight, y = Embargo, group = Embargo, color = Embargo)) +
  geom_boxplot() +
  facet_grid(. ~ Year) + 
  coord_flip() +
